    ...
    require(["digg_utils"], (utils) => {
      ...
      if (paths.includes("topic")) {
        ...
        if (utils.isNumeric(endPath)) {
          ...
          const first = post.find(focusableElements)[0];
          first && first.focus();
        }
      }
      /* #endregion */
      ...
      $("#thread_tools_button").on("click", (e) => {
        setTimeout(() => {
          ...
          const threadModalTriggers = threadTools.find(
            "a[component='topic/move'], a[component='topic/delete'], a[component='topic/pin']"
          );
          const threadPanelTriggers = threadTools.find(
            "a[component='topic/fork'], a[component='topic/move-posts'], a[component='topic/delete/posts']"
          );
      ...
      $("#post_tools_button").on("click", (e) => {
        setTimeout(() => {
          ...
          const postModalTriggers = postTools.find(
            "a[component='post/delete'], a[component='post/view-history'], a[component='post/flag']"
          );
          const postPanelTriggers = postTools.find(
            "a[component='post/change-owner']"
          );
          const postComposerTriggers = postTools.find(
      ...
      function handleComposer(referrer) {
        setTimeout(() => {
          // Timeout effects are used to wait for components to mount
          ...
          setTimeout(() => {
            const focusable = composer.find(
              'button:not(.btn-sm), textarea, input[type="text"]:not(".tags")'
            );
            const dropDown = composer.find(".dropdown-toggle");
            const input = composer.find(
              ".title, .write, .ui-autocomplete-input"
            );
      ...
      function trapFocus(element, focusableEl, nextToFocus, skipToMainContent) {
        ...
        element.addEventListener("keydown", (e) => {
          ...
          const escapePressed =
            e.key === "Escape" || e.keyCode === KEYCODE_ESCAPE;

          escapePressed &&
            clickOutside(element, nextToFocus, skipToMainContent);
          ...
          if (e.shiftKey) {
            ...
            if (
              ...
              (lastFocusable &&
                lastFocusable.disabled &&
                document.activeElement === secondToLast) ||
              (lastFocusable &&
                lastFocusable.disabled &&
                secondToLast &&
                secondToLast.disabled &&
                document.activeElement === thirdToLast)
            ) {
              firstFocusable.focus();
              e.preventDefault();
            }
      ...
      function handleModal(referrer, composerReferrer) {
        setTimeout(() => {
          ...
          const focusable = modalContent.find(
            "a[href]:not(.hidden), button, select, input"
          );
          ...
          $(focusable).on("click", (e) => {
            setTimeout(() => {
              const isAborting =
                $(e.target).hasClass("bootbox-cancel") ||
                $(e.target).hasClass("bootbox-close-button");
      ...
      function handlePanel(referrer) {
        setTimeout(() => {
          ...
          const focusable = panelContent.find(
            "a[href]:not(.hidden), button, select, input"
          );
      ...
      function onModalClose(referrer) {
        setTimeout(() => {
          ...
          if (modalContent.length == 0) {
            const dropdown = $(referrer)
              .parent()
              .parent()
              .siblings("[data-toggle='dropdown']");
            dropdown.length > 0
              ? dropdown.focus()
              : refferrer && referrer.focus();
          }
        }, 300);
      }
